 ...
 */
require('./mricode.pagination.css');
(function (factory) {
    factory($);
})(function ($) {
    "use strict";
    ...
    var Page = function (element, options) {
        ...
        this.$page = $('<ul class="m-pagination-page m-page-ul"></ul>').hide();
        this.$size = $('<div class="m-pagination-size m-page-size"></div>').hide();
        this.$jump = $('<div class="m-pagination-jump m-page-jump"></div>').hide();
        this.$info = $('<div class="m-pagination-info m-page-info"></div>').hide();
        ...
        */
        ...
        renderInfo: function (currentPageIndex, currentPageSize, total, infoFormat, noInfoText) {
            if (total <= 0) {
                ...
                var pageSize = Math.ceil(total/currentPageSize);
                // var startNum = (currentPageIndex * currentPageSize) + 1;
                // var endNum = (currentPageIndex + 1) * currentPageSize;
                // endNum = endNum >= total ? total : endNum;
                return infoFormat/*.replace('{start}', startNum).replace('{end}', endNum)*/.replace('{pageSize}', pageSize).replace('{total}', total).replace('{index}',currentPageIndex+1);
            }
        },
        //计算最大分页数
