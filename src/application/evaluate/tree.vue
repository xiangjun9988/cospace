  <template>
    <ul class="tree">
        <template v-for="(item, index) in treedata">
            <li :class="{'show-child': item.showChild, 'active': ischeckbox && activedata.indexOf(item.moduleId) !==-1}">
                <a href="javascript:;" :class="{'active': !ischeckbox && activedata.indexOf(item.moduleId) !==-1}">
                    ...
                    <div class="radio">
                        <label class="no-padding-left">
                            ...
                            <input name="menuCheckbox" type="checkbox" class="colored-blue" v-if="ischeckbox" @click="handleCallback($event,item)" :checked="activemoduleid.indexOf(item.moduleId) != -1">
                            <span class="text" :title="item.moduleName">{{item.moduleName}}</span>
                        </label>
                    </div>                                         
