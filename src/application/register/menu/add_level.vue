...
export default{
    ...
    methods: { 
        confirm(){
            ...
            $.post(window.config.api + "/module/save.do?token="+token,{appCode:this.$route.query.appCode,parentId:0,moduleName:this.menuData.moduleName,moduleUrl:this.menuData.moduleUrl,icon:this.menuData.icon,isOrder:this.menuData.isOrder,isUse:this.menuData.isUse,remark:this.menuData.remark})
            .then(data=>{  
                if(!data.success){
                    //返回错误状态 
                    $.dialog('数据保存失败！');                   
