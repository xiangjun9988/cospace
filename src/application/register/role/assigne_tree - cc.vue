  <template>
    <ul class="tree">
        <template v-for="(item, index) in treedata">
            <li :class="{'show-child': item.showChild, 'active': ischeckbox && activedata.indexOf(item.id) !==-1}" v-on:click="bindRole($event,item)">
                <a href="javascript:;" :class="{'active': !ischeckbox && activedata.indexOf(item.id) !==-1}">
                    ...
                    <div class="radio">
                        <label class="no-padding-left">
                            <span class="text" :title="item.parentName+ '/' +item.name"><span v-if="item.parentName">{{item.parentName || ""}}/</span>{{item.name}}</span>
                        </label>
                    </div>                                         
                </a>
                <tree-view 
