                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <form class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-lg-6">
                                    <input type="text" class="form-control" v-model="roleData.roleName" placeholder="请输入角色名称"  v-on:change="checkName">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-lg-3 control-label">角色编码<span class="red margin-left-5">*</span></label>
                                <div class="col-lg-6">
                                    <input type="text" class="form-control" v-model="roleData.roleCode" placeholder="请输入角色编号" v-on:change="checkCode">
                                </div>
                            </div>                            
                            <div class="form-group">
...
export default{
    ...
    methods: {
        checkName(){
            ...
            $.post(window.config.api + "/role/queryRoleByParams.do?token="+token,{roleName:this.roleData.roleName,appCode:this.$route.params.id})            
            .then(data=>{
        ...
        checkCode(){
            let token = localStorage.getItem('token');
            $.post(window.config.api + "/role/queryRoleByParams.do?token="+token,{roleCode:this.roleData.roleCode,appCode:this.$route.params.id})            
            .then(data=>{
                if(!data.success){
                    //返回错误状态 
                    $.dialog(data.msg);                   
        ...
        confirm(){
            ...
            .then(data=>{
                ...
                .then(data=>{
                    ...
                    //新增此分类下角色
                    $.post(window.config.api + "/role/save.do?token="+token,{roleName:this.roleData.roleName,roleCode:this.roleData.roleCode,remark:this.roleData.remark,isUse:this.roleData.isUse,appCode:this.$route.params.id})            
                    .then(data=>{
                        if(!data.success){
                            $.dialog(data.msg);                    
                            return false;
