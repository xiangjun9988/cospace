...
export default {
  ...
  created: function() {
    ...
    //检测数据变化
    this.$watch('$route.fullPath', this.refresh, this.searchList);
  },
  methods: {
    refresh(){
      let token = localStorage.getItem('token');
      //console.log(this.roleName)
      $.get(window.config.api + "/sysParameterConfig/queryListByPage.do?token="+token,{paramName:this.$route.query.searchName,currentPage:this.$route.query.currentPage, pageSize:this.$route.query.pageSize, appCode:this.$route.query.appCode})
      .then(data=>{
          if(!data.success){
              //返回错误状态                    
              return false;
          ...
          if(this.dataList == undefined && this.queryData.currentPage != 1){
              Object.assign(this.queryData, {currentPage:this.queryData.currentPage-1});
              this.refreshRouter();
          }
          if(this.dataList == undefined && this.queryData.currentPage == 1){
    ...
    searchList(){
        Object.assign(this.queryData, {currentPage:1, searchName:this.searchName});
        this.refreshRouter();
    },
    reset(){
        Object.assign(this.queryData, {currentPage:1,pageSize:10, searchName:""});
        this.searchName = "",
        this.refreshRouter();
    },
