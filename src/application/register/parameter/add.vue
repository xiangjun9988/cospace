                    ...
                    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 col-lg-push-1 col-md-push-1 col-sm-push-1 col-xs-push-1">
                        <form class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-lg-4">
                                    <input type="text" class="form-control" placeholder="请输入参数键" v-model="parameter.paramKey" v-on:blur="checkParamKey">
                                </div>
                                <label class="col-lg-2 control-label"><i class="red margin-left-5">*</i>应用编码：</label>
                                <div class="col-lg-4">
                                    <div style="margin-top: 7px;">{{$route.query.appCode}}</div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-lg-2 control-label"><i class="red margin-left-5">*</i>参数名称:</label>
                                <div class="col-lg-10">
                                    <input type="text" class="form-control" placeholder="请输入参数名称" v-model="parameter.paramName" v-on:blur="checkParamName">
                                </div>
                            </div>  
                            <div class="form-group">
...
export default{
    ...
    methods: {
        ...
        checkParamKey(){
            ...
            $.post(window.config.api + "/sysParameterConfig/checkParamKey.do?token="+token,{paramKey:this.parameter.paramKey})    
            .then(data=>{ 
                if(!data.success){
                    //返回错误状态 
                    $.dialog(data.msg);    
        ...
        checkParamName(){
            let token = localStorage.getItem('token');
            $.post(window.config.api + "/sysParameterConfig/checkParamName.do?token="+token,{paramName:this.parameter.paramName})
            .then(data=>{
                if(!data.success){
                    //返回错误状态 
                    $.dialog(data.msg);
        ...
        confirm(){
            ...
            let token = localStorage.getItem('token');
            $.post(window.config.api + "/sysParameterConfig/save.do?token="+token,{paramKey:this.parameter.paramKey, appCode:this.$route.query.appCode, paramName:this.parameter.paramName, paramValue:this.parameter.paramValue, isOrder:this.parameter.isOrder, isUse:this.parameter.isUse, remark:this.parameter.remark})            
            .then(data=>{
                if(!data.success){
                    //返回错误状态 
                    $.dialog(data.msg);                   
