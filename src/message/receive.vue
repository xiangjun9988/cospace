	    ...
	    <div class="page-body">
			<div class="row">
		    	<div class='col-xs-12'>
		    		...
		    		<div style="overflow:hidden;"><div class="pull-right">
	                                    ...
	                                    <table class="table table-striped table-bordered table-hover" id="vueTable">
	                                        <thead class="bordered-blue">
	                                             <tr>
							                        ...
							                        <th class="text-center"  width='10%'>发送者</th>
	                                        ...
	                                        <tbody>
	                                            <tr v-for='(item, index) in receiveMessages'>
													...
													<td class="text-center">{{item.messageAuthorName}}</td>
													<td class="text-center">{{item.sendDate}}</td>
													<td class="text-center">{{item.messageSource}}</td>
													<td class="text-center"><!-- <a href="javascript:void(0)">删除</a> --></td>
							                    </tr>
	                                        </tbody>
	                                    </table> 
	                                </div> 
	...
	methods : {
		refresh(){
	        ...
	        .then(data=>{
	        	...
	        	messageRecId = data.obj.userMap.userId;
	        	//alert(messageRecId)
	        	$.get(window.config.api + "/sorMessage/queryListByPage.do?token="+token,{currentPage:this.currentPage,pageSize:this.pageSize, messageRecId:messageRecId, messageContent: this.messageContent})
		        .then(data=>{
		            if(!data.success){
		                //返回错误状态 
		                //alert(1)                   
