  <template>
    <ul class="tree">
        <template v-for="(item, index) in treedata">
            <li :class="{'show-child': item.showChild, 'active': ischeckbox && activedata.indexOf(item.id) !==-1}"  @click="handleCallback($event,item)">
                <a href="javascript:;" :class="{'active': !ischeckbox && activedata.indexOf(item.id) !==-1}">
                    ...
                    <div class="radio">
                        <label class="no-padding-left">
                            <input name="department" type="radio" class="colored-blue" v-if="isradio && !item.childrenList">
                            <input name="department" type="checkbox" class="colored-blue" v-if="ischeckbox">
                            <span class="text" :title="item.name">{{item.name}}</span>
                        </label>
                    </div>                                         
                    <span class="handle" v-if="hasadd || hasdel">
                        <i class="blue glyphicon glyphicon-plus-sign margin-right-10 tree-add" @click="treeAdd($event, item)" v-if="hasadd"></i>
