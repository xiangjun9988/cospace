  <template>
    <ul class="tree">
        <template v-for="(item, index) in treedata">
            ...
            v-if="childListShow(item) || childshow">
                <a href="javascript:;" :class="{'active': !ischeckbox && activedata.indexOf(item.id) !==-1}" >
                    ...
                    <div class="radio">
                        <label class="no-padding-left">
                            <input name="radio" type="radio" class="colored-blue" v-if="isradio" @click="handleCallback($event,item)">
                            <input type="checkbox" class="colored-blue" v-if="ischeckbox" @click="handleCallback($event,item)" :checked="activeid.indexOf(item.id) != -1">
                            <span class="text" :title="item.areaName">{{item.areaName}}</span>
                        </label>
                    </div>                                         
                ...
                <tree-view 
                    ...
                    :childshow="childshow?childshow:JSON.stringify(item.areaName).indexOf(searchareaname) != -1">
                </tree-view>
            </li>
        </template>
    </ul>
...
export default {
    ...
    methods: {
        childListShow(item){
            //(JSON.stringify(item.areaName).indexOf(searchareaname) != -1?true:false)
            // if(JSON.stringify(item.areaName).indexOf(this.searchareaname) == -1){
            //     this.childshow = true;
            // }
            if(JSON.stringify(item).indexOf(this.searchareaname) == -1){
                return false;
            }
            if(JSON.stringify(item).indexOf(this.searchareaname) != -1){
                return true;
